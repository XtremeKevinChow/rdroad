	...
	<tr>
			...
			<td class="queryButton">
                <input type="image" style="border-width: 0px;" src="../Images/search.gif" id="sel_cmdQuery" />
					<input type="hidden" style="display:none"; id="hidOrgId" value="" />
            </td>		 
    <tr>        
</table>
...
<table id="dataArea" cellpadding="0" cellspacing="0" style="width: 100%; height: 100%">
    ...
    <%
        ...
        string orgId = WebUtil.Param("orgId");
        ...
        using (ISession session = new Session())
        {
            ...
            EntityQuery query = session.CreateEntityQuery<User>();
           	if(!String.IsNullOrEmpty(userName))
			{
				query.And(Exp.Like("UserName", "%" + userName+ "%")); 
			}
           	if(!String.IsNullOrEmpty(fullName))
			{
				query.And(Exp.Like("FullName", "%" + fullName+ "%")); 
			}
           	if(!String.IsNullOrEmpty(orgId))
			{
				query.And(Exp.Eq("OrgId", orgId)); 
			}
            query.SetPage(index, 9);
...
function User_on_load(params){
	...
	$("#hidOrgId", theForm).val(params.orgId); 
...
function User_on_query(params){
	params.userName = $("#txtUserName", $(this.mfs_formId)).val(); 
	params.fullName = $("#txtFullName", $(this.mfs_formId)).val(); 
	params.orgId = $("#hidOrgId", $(this.mfs_formId)).val(); 
}
//确认按钮点击事件，返回选择的结果
function User_on_select(params){
    return params.selected;
}
//关闭按钮事件
function User_on_close(){
}
</script>
