<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MsgTemplateManage.aspx.cs" Inherits="MsgTemplateManagePage" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">  
	  ...
	  //Nav to Edit Page
	  function Edit(tmplCode, subscribe)
	  {	  
	         var url = "MsgTemplateEdit.aspx?";
	         if(subscribe)
	            url = "MsgSubscriberManage.aspx?";
	            
	        if(tmplCode)
	        {
	            url +="mode=edit";
	            url += "&tmplCode="+ tmplCode;
	        }
	        else
	        {
	            url +="mode=new" ;
...
<body>
    ...
    <div class="queryPage">
        <input type="hidden" id="hidReturnUrl" runat="server" value="MsgTemplateManage.aspx" /> 
        <uc1:FunctionTitle ID="functionTitle" runat="server" PageTitle="消息模板管理" />
        <table class="queryArea" cellpadding="1" cellspacing="1">
		 <tr>
                <td class="label">消息模板代码</td>
                <td><asp:TextBox runat="server" ID="txtTmplCode"  ></asp:TextBox></td>
		...
		<tr>
                ...
                <td><asp:CheckBoxList runat="server" ID="cklAccessibility"  RepeatDirection="Horizontal"  ></asp:CheckBoxList></td>
<td></td>
	     <td></td><td></td>
        ...
        <div class="gridArea">
            ...
            <table class="datalist" id="data_list_table" border="0" cellpadding="0" cellspacing="1"
                ...
                <asp:Repeater ID="rptMsgTemplate" runat="server">
                <ItemTemplate>
                     <tr>
                            <td>
                                <input id="checkbox"   tmplCode='<%# Eval("TmplCode") %>'   type="checkbox" runat="server" />
                            </td>
                            <td>
                                <a href="javascript:Edit('<%# Eval("TmplCode")%>')" >编辑</a>
                            </td>
                            <td>
                                <a href="javascript:Edit('<%# Eval("TmplCode") %>',true)" title="订阅"><img alt="订阅" src="../images/people.gif"></a>
                            </td>                            
							<td>	<%# Eval("TmplCode") %></td>
							<td>	<%# Eval("Name") %></td>
							<td>	<%# WebUtil.FindTextByValue(this.ddlMsgType,Eval("MsgTypeId"),false) %></td>
