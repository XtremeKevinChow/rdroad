...
public partial class Inventory_ProductOutManage : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                OrderTypeDef typeDef = OrderTypeDef.Retrieve(session, StockInHead.ORD_TYPE_PRD_OUT);
                if (typeDef != null)
                    this.hidViewUrl.Value = typeDef.ViewURL;

                OrderStatusDef statusDef = OrderStatusDef.Retrieve(session, StockInHead.ORD_TYPE_PRD_OUT, (int)StockInStatus.New);
                this.chkNew.Text = statusDef.StatusText;
                statusDef = OrderStatusDef.Retrieve(session, StockInHead.ORD_TYPE_PRD_OUT, (int)StockInStatus.Release);
                this.chkRelease.Text = statusDef.StatusText;
                statusDef = OrderStatusDef.Retrieve(session, StockInHead.ORD_TYPE_PRD_OUT, (int)StockInStatus.Open);
                this.chkOpen.Text = statusDef.StatusText;
                statusDef = OrderStatusDef.Retrieve(session, StockInHead.ORD_TYPE_PRD_OUT, (int)StockInStatus.Close);
...
order by CreateTime desc")
            ...
            .SetValue("?orderType", StockInHead.ORD_TYPE_PRD_OUT, "h.OrderTypeCode")
    ...
    protected void MagicItemCommand(object sender, MagicItemEventArgs e)
    {
        using (ISession session = new Session())
        {
            ...
            try
            {
                ...
                if (deleted)
                {
                    WebUtil.ShowMsg(this, "选择的领用单已经删除");
