...
string m = WebUtil.Param("m").Trim().ToLower();
bool showPrice = m == "sp";
...
%>
    <div>
        <h4 style="margin:0;">
            <%=head.OrderTypeCode == StockInHead.ORD_TYPE_PRD_IN?"产品入库单":"产品领用单"%>
            <%= head.OrderNumber%>
        </h4>
        <table class="queryArea" cellpadding="0" cellspacing="0" style=" margin-bottom:8px;">
            <tr>
                ...
                <td class="label" style="width:70px;">领用人：</td>
                <td style="width:75px;background-color:#fff;"><%= head.Responsible %>&nbsp;</td>
            ...
            <tr>
                ...
                <td class="label">引用单据：</td>
                <td style="background-color:#fff;"><%= head.RefOrdNum%></td>
        ...
        <table class="datalist2" cellpadding="0" cellspacing="0">
                <tr>
                    ...
                    <td class="h" style='width: 80px;'>金额</td>
                </tr>
<%
    DataSet ds = session.CreateObjectQuery(@"
select s.BarCode as BarCode,m.ItemCode as ItemCode,m.ItemName as ItemName,s.ColorCode as ColorCode,s.SizeCode as SizeCode
...
order by l.LineNumber")
    ...
    {
        color = "";
        totalAmt += Cast.Decimal(row["Price"]) * Cast.Decimal(row["Quantity"]);
...
%>
                <tr style='color:<%= color %>'>
                    ...
                    <td style="text-align:right;padding-right:4px;"><%= !showPrice ? "" : RenderUtil.FormatNumber(Cast.Decimal(row["Price"]) * Cast.Decimal(row["Quantity"]), "#0.#0", "&nbsp;")%></td>
                </tr>
<% 
    }
%>
<% if (showPrice)
   { %>
                <tr>
                    <td colspan="10" style="text-align:right;">总金额&nbsp;</td>
