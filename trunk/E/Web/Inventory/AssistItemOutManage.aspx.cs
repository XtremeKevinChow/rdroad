...
public partial class Inventory_AssistItemOutManage : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            using (ISession session = new Session())
            {
                ...
                OrderStatusDef statusDef = OrderStatusDef.Retrieve(session, StockInHead.ORD_TYPE_ASSIST_OUT, (int)StockInStatus.New);
                this.chkNew.Text = statusDef.StatusText;
                statusDef = OrderStatusDef.Retrieve(session, StockInHead.ORD_TYPE_ASSIST_OUT, (int)StockInStatus.Release);
                this.chkRelease.Text = statusDef.StatusText;
                statusDef = OrderStatusDef.Retrieve(session, StockInHead.ORD_TYPE_ASSIST_OUT, (int)StockInStatus.Open);
                this.chkOpen.Text = statusDef.StatusText;
                statusDef = OrderStatusDef.Retrieve(session, StockInHead.ORD_TYPE_ASSIST_OUT, (int)StockInStatus.Close);
...
order by CreateTime desc")
            ...
            .SetValue("?orderType", StockInHead.ORD_TYPE_ASSIST_OUT, "h.OrderTypeCode")
