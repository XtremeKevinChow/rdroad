...
public partial class Inventory_ProductOutLine : System.Web.UI.Page
{
    ...
    private IList<WHArea> GetAreas()
    {
        ...
        if (this._areas == null)
        {
            this._areas = ERPUtil.GetWHArea(this._session, StockInHead.ORD_TYPE_PRD_OUT, null);
        }
        return this._areas;
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            this.txtReturnUrl.Value = "ProductOutLine.aspx?ordNum=" + this.OrderNumber + "&return=" + WebUtil.escape(WebUtil.Param("return"));
        ...
        else if (e.CommandName == "Close")
        {
            ...
            using (ISession session = new Session())
            {
                try
                {
                    ...
                    head.Close(session);
                    session.Commit();
                    WebUtil.ShowMsg(this, "产品领用单" + head.OrderNumber + "已经完成");
                    this.QueryAndBindData(session, head);
                    this.SetView(head);
                }
                catch (Exception er)
