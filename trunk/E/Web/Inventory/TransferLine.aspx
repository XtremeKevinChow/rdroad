<%@ Page Language="C#" AutoEventWireup="true" CodeFile="TransferLine.aspx.cs" Inherits="Inventory_TransferLine" %>
...
function ondelete(){
    ...
    return confirm("确定要删除选择的移库明细？");
}

function onrelease(){
    return confirm("确信发布移库单？");
}
function onclose(){
    return confirm("确信关闭移库单？");
}

function AddNew(){
    var url = $("#txtUrlToThisPage").val();
    window.location.href = "TransferNewLine.aspx?ordNum=" + $("#txtOrderNumber").val() + "&return=" + escape(url);
}
</script>
</head>
<body>
                ...
                <asp:Repeater ID="repeatControl" runat="server">
                    <ItemTemplate>
                        <tr>
                            ...
                            <td style="width:40px;">
                                <%# RenderUtil.FormatString(Eval("FromArea"))%>
                            </td>
                            <td style="width:75px;">
                                <%# RenderUtil.FormatString(Eval("FromSection"))%>
                            </td>
                            <td style="width:55px;">
                                <%# RenderUtil.FormatString(Eval("MoveQty"))%>
                            </td>
                            <td style="width:55px;">
                                <%# RenderUtil.FormatString(Eval("ToArea"))%>
                            </td>
                            <td style="width:75px;">
                                <%# RenderUtil.FormatString(Eval("ToSection"))%>
                            </td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
