...
/*得到8位数的随机数字*/
/* add bymagic at 2007-05-29 */
public   static   final   StringBuffer   getRadom(int   iLen,int   iType){ 
    ...
    switch   (iType)   {   
        case   0   :   //:表示仅获得数字随机数
            for   (int   iLoop=0;iLoop<iLen;iLoop++){ 
            	...
            	strRandom = strRandom.append(Integer.toString(rnd.nextInt(10)));
            }   
            break;   
        case   1   :   //表示仅获得字符随机数
            for   (int   iLoop=0;iLoop<iLen;iLoop++){   
                strRandom = strRandom.append(Integer.toString((35-rnd.nextInt(10)),36));   
            }   
            break;   
        case   2   :   //表示获得数字字符混合随机数   
            for   (int   iLoop=0;iLoop<iLen;iLoop++){   
                strRandom = strRandom.append(Integer.toString(rnd.nextInt(36),36));   
            }   
            break;   
    }   
    return   strRandom;   
...
/*
RandomStr.setCharset("0-z");//指定字符内容为"0"-"z"，当然也可以是别的，比如"a-z"或"A-Z" 

RandomStr.setLength("20");//指定字符长度 
RandomStr.generateRandomObject();//生成随机数 

String str = RandomStr.getRandom();//得到随机数 
