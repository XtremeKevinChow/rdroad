    ...
    <tr>
    	...
    	<td>
			<b><font color="838383">当前位置</font></b><font color="838383"> : </font><font color="838383">数据分析</font><font color="838383"> 
      		-&gt; </font><font color="838383">产品日销售统计</font><font color="838383">&nbsp; 
      	</font></td>
   </tr>
</table>
<form   action="sale_sku_daily_stats.jsp" method="post" name="form" onsubmit="return querySubmit();">
...
<table width="95%" align="center" cellspacing="1" border="0"  class="OraTableRowHeader" noWrap id="data">
	<tr>
		...
		<th width="10%" class="OraTableRowHeader" noWrap  noWrap align=middle>平均单价</th>
...
<%
		if(begin_date.length()>0){
		   condition+=" and t1.release_date >= date '"+begin_date+"' and t1.release_date< 1+date'" + begin_date + "'";
		}
		String count_sql=			
            " select t3.itm_code,t3.COLOR_CODE,t3.SIZE_CODE,t4.itm_name,"
            +" sum(t2.quantity*t2.price) sum_amt,sum(t2.quantity) sum_qty,round(sum(t2.quantity*t2.price)/sum(t2.quantity),2) avg_price,"
            +" t7.use_qty,t5.catalog_name,t6.name"
            ...
            +" group by t3.itm_code,t3.COLOR_CODE,t3.SIZE_CODE,t4.itm_name,t7.use_qty,t5.CATALOG_NAME,t6.name";
		

        request.getSession().setAttribute("excel_name","产品日销售统计");
        request.getSession().setAttribute("excel_title","货号,颜色,尺寸,名称,总金额,总数量,平均单价,现有库存,类别,订单来源");
...
%>
	<tr>
		...
		<td bgcolor="#FFFFFF"><%=rs.getDouble("sum_amt")%></td>		
		<td bgcolor="#FFFFFF"><%=rs.getInt("sum_qty")%></td>
		<td bgcolor="#FFFFFF"><%=rs.getDouble("avg_price")%></td>	
