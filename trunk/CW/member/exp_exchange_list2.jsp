...
function exchange_f(obj) {
	...
	for (i = 1; i < len; i ++) {
		...
		{
			selectItem = row.getElementsByTagName("INPUT")(0).value;
			flag = true;
			break;
		}
	...
	if (flag == true)
	{
		if (confirm("确实要兑换吗?"))
		{
			...
			//{
				//document.forms[0].action = "/order/orderAddSecond.do?type=addExchangeGift2&isAdd=<%=isAdd%>";
				//document.forms[0].target="main";
				//document.forms[0].submit();

				window.close();
				self.opener.exchange(selectItem, document.forms[0].memberID.value,document.forms[0].cardID.value, "<%=isAdd%>");
			//} else {
				//document.forms[0].action = "/member/expExchange.do?type=expChangeGift2";
...
<html:form action="/expExchange.do?type=queryValidGift2" onsubmit="return doSearch();">
...
<table  align="center" width="95%" border=0 cellspacing=1 cellpadding=5 >
	<tr>
		...
		&nbsp;
		<input type="submit" name="query" value=" 一次性兑换查询 " >
		</td>
	...
	<tr>
		<td><span class="style1">会员姓名：</span><bean:write name="expExchangeForm" property="memberName"/>&nbsp;&nbsp;&nbsp;&nbsp;
			<span class="style1">本年度剩余积分：</span><bean:write name="expExchangeForm" property="amountExp"/>
