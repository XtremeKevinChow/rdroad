...
DecimalFormat myformat = new DecimalFormat("###,###.00");
			...
			String s_pro_no=request.getParameter("s_pro_no");					 			 						 						 
...
<table width="99%" align="center" border=0 cellspacing=1 cellpadding=5 class="OraTableRowHeader" noWrap >
	<tr>
		...
		<th width="6%"  class="OraTableRowHeader" noWrap  noWrap align=middle>供应商代码</th>
		<th width="6%"  class="OraTableRowHeader" noWrap  noWrap align=middle>供应商名称</th>
...
<%
		...
		try{
		 conn = DBManager.getConnection();
			...
			sql="select a.*, d.pro_no, d.pro_name,c.createdate,c.invoice_date,c.fact_ap_code ";			
			sql+=" from fin_ap_dtl a ";
		      ...
		      if(s_pro_no.length()>0){
		      			sql+=" and c.pro_no ='"+s_pro_no+"'";
		      }      
      //out.println(sql);
			...
			while(rs.next()){	
				  ...
				  double purPrice_amt=Arith.round(purPrice*qty,2);//预算成本
					...
					double taxAmt=Arith.round(totalAmt-amt,2);//税金金额					
					
%>	
	<tr align=center>

		<td bgcolor="#FFFFFF" noWrap align="left"><%=pro_no%></td>
		<td bgcolor="#FFFFFF" noWrap align="left"><%=pro_name%></td>
		<td bgcolor="#FFFFFF" noWrap align="left"><%=invoice_date%></td>
		<td bgcolor="#FFFFFF" noWrap align="left"><%=fact_ap_code%></td>	
		<td bgcolor="#FFFFFF" noWrap align="left"><%=createdate%></td>	
		...
		<td bgcolor="#FFFFFF" noWrap align="right"><%=myformat.format(totalAmt)%></td>
</tr>
<%}%>
