function loadTree(settings){
    ...
    mm.mask( { } )
        .__popup( { pop_src: "#ajaxStatus", pop_target: "#divTree", sysFrame: false });
    $.ajax({
        async: true,
...
function setupTree(settings){
    if(settings.type=="mgmt"){
        ...
        $(settings.resultid).find('span').contextMenu("treeMenu", {
            ...
            bindings: { "cmdInsert": function(t) { clearView("create", $(t).attr("oid")); },  "cmdDelete": function(t) { cmdDelete(t, settings);} }
...
function saveGroup(settings){
    if(!saveHandler()) return;
    settings.resultid="#userGroup";
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            mm.msg( {msg:data.desc+" 保存成功", title:"操作成功", timeout: 0.6, width: 180, valign: "top-in", vmargin:100 } );
        },
        error: function(data, msg, e) {
            alert(e + " " + msg + " " + data);
            mm.__close("#ajaxStatus").maskClose();
...
function afterSave(orgInfo, isUpdate, isInsert, settings){
    setView(orgInfo);
    if(isUpdate)
...
} //function afterSave(orgInfo)

function userGroupSelect(){}
userGroupSelect.prototype.exec=function(callback, settings){
    //mm.mask({});
    $.ajax({
        async: false,
        success: function(data) {
        ...
        data: { type: settings.query, action:"SelectUserGroup" },
        dataType: "json",
        timeout: 5000,
        type: "POST",
        url: "../ajax.ashx"
...
function msgSubscriberUserGroupSelect(){}
msgSubscriberUserGroupSelect.prototype.exec=function(callback, settings){
    //mm.mask({});
    $.ajax({
        async: false,
        success: function(data) {
        ...
        data: { type: settings.query,msgtmpl:settings.msgtmpl, action:"MsgSubscriberSelectUserGroup" },
        dataType: "json",
        timeout: 5000,
        type: "POST",
        url: "../ajax.ashx"
...
window.userGroup_close=window.partner_close=function(settings){
