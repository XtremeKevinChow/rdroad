...
;(function($) {
	...
	$.extend($.fn, {
		...
		//removed by Richie, 
		//collapse/expand the subtree nodes by clicking on the hitarea div, instead of on span node
		//when clicking on span node, turn it to selected status
...
//			this.filter(":has(>ul):not(:has(>a))").find(">span").add( $("a", this) ).hoverClass();
		...
		treeview: function(settings) {
			...
			treeitems.find(">span").bind("click", function(){
			    if($.className.has($(this)[0], CLASSES.selected)) return; //if it has already in selected state, just do nothing
