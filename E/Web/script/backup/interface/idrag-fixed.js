...
jQuery.iDrag =	{
	...
	hidehelper : function(dragged)
	{
		...
		//Modified by Richie, January 29, 2008
		//The original code will cause all events binded to the source elements, whose whole content is cloned and appended to the helper,
		//    to be lost in case of IE (The jQuery do this and maybe unfortunately meet the IE's bugs, so I fix this by using the removeChild directly,
		//    Perhaps it will brings memory leak just as jQuery says, but no better solutions till now)
		//jQuery.iDrag.helper.empty().hide().css('opacity', 1);  //the original code
		jQuery.iDrag.helper[0].removeChild(jQuery.iDrag.helper[0].firstChild); //if multiple elements in drag helper?
