...
public partial class Receive_MemberReturnManage : System.Web.UI.Page
{
...
select h.OrderNumber as OrderNumber,h.CreateTime as CreateTime,h.Status as Status,sd.StatusText as StatusText,h.ApproveResult as ApproveResult
    ...
    ,u.FullName as UserName,td.TypeText as TypeText,h.OrderTypeCode as OrderTypeCode,h.CSUser as CSUser
from ReturnHead h
left join OrderTypeDef td on h.OrderTypeCode=td.OrderTypeCode
left join OrderStatusDef sd on h.OrderTypeCode=sd.OrderTypeCode and h.Status=sd.StatusValue
left join User u on u.UserId=h.CreateUser
left join Member m on m.MemberID=h.MemberID
where h.OrderTypeCode in (?mbrReturn, ?exchangeReturn)
order by h.CreateTime desc")
            .Attach(typeof(ReturnHead)).Attach(typeof(OrderStatusDef)).Attach(typeof(OrderTypeDef))
            .Attach(typeof(User)).Attach(typeof(Member))
            .SetValue("?mbrReturn", ReturnHead.ORDER_TYPE_MBR_RTN, "h.OrderTypeCode")
            .SetValue("?exchangeReturn", ReturnHead.ORDER_TYPE_EXCHANGE_RTN, "h.OrderTypeCode")
        ...
        if (this.txtSNNumber.Text.Trim().Length > 0)
            query.And(Exp.Like("h.RefOrderNumber", "%" + this.txtSNNumber.Text.Trim() + "%"));
