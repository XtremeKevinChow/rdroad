<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ProductOutLine.aspx.cs" Inherits="Inventory_ProductOutLine" %>
...
function onadd(){
    ...
    window.location.href = "ProductOutNewLine.aspx?ordNum=" + $("#txtOrderNumber").val()
...
function onrelease(){
    return confirm("确信发布产品领用单？");
}
function onclose(){
    return confirm("确信关闭产品领用单？");
}
function onprint() {
    if ($.trim($("#txtOrderNumber").val()).length <= 0) return;
...
<body>
    ...
    <div>
        <uc1:FunctionTitle ID="functionTitle" runat="server" PageTitle="产品领用明细" />
