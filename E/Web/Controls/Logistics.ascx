...
<table id="dataArea" cellpadding="0" cellspacing="0" style="width: 100%; height: 100%">
    ...
    <%
        ...
        using (ISession session = new Session())
        {
            ...
            EntityQuery query = session.CreateEntityQuery<Logistics>()
            ...
            IList<Logistics> items = query.List<Logistics>();
            if (queryType == 1)
                count = (query.Count()+8) / 9;

            foreach (Logistics item in items)
            {
    %>
    <tr>
        <td style="width:20px;">
            <input type="checkbox" itemId='<%=item.LogisticCompID %>' itemDesc='<%=item.ShortName %>' class="chk" />
        </td>
...
function Logistics_on_load(params){
    var thisRef=this;
    ...
    if(params.selected && params.selected.constructor==Array)
        $("#dataArea", theForm).find(".chk").each(function(i, e){
            var t = $.grep(params.selected, function(n, i){ return n.logisticsId==$(e).attr("itemId"); });
            if(t.length>0) {
                $(e).attr("checked", true);
                t[0].logisticsId = $(e).attr("itemId"); 
                t[0].logisticsName = $(e).attr("itemDesc"); 
            }
            $(e).bind("click", function(){
                var isChecked = $(this).attr("checked");
                if(thisRef.settings.mode=="s") { 
                    if(isChecked) {					
                        params.selected = [ { logisticsId:$(this).attr("itemId") ,logisticsName:$(this).attr("itemDesc") } ];
                        $("#dataArea", theForm).find(".chk" + "[@itemId!='" +$(this).attr("itemId")+"']").removeAttr("checked");
                    }
                    else params.selected = [] ;
                }else { 
                    if(isChecked)
                        params.selected.push({ logisticsId:$(this).attr("itemId") ,logisticsName:$(this).attr("itemDesc") });
                    else 
                        params.selected = $.grep(params.selected, function(n, i){
                            return n.logisticsId==$(e).attr("itemId");
                        });
                }
            });//$(e).bind("click", function()
