...
<bean:define id="member" name="cart" property="member" type="com.magic.crm.member.entity.Member"/>
 ...
 <input type="hidden" name="cardId" value="<bean:write name="member" property="CARD_ID"/>">

<table width="95%" align="center" cellspacing="0" border="0"  >
	<tr>
		<td>
			电话<html:radio property="prTypeId" value="2" onclick="ajaxpage2('../order/orderAddSecond.do?type=selectPrType','ajaxcontentarea',document.forms[0])"/>
			&nbsp;&nbsp;
			邮购<html:radio property="prTypeId" value="1" onclick="ajaxpage2('../order/orderAddSecond.do?type=selectPrType','ajaxcontentarea',document.forms[0])"/>&nbsp;&nbsp;&nbsp;&nbsp;
			传真<html:radio property="prTypeId" value="6" onclick="ajaxpage2('../order/orderAddSecond.do?type=selectPrType','ajaxcontentarea',document.forms[0])"/>&nbsp;&nbsp;&nbsp;&nbsp;
			Email<html:radio property="prTypeId" value="7" onclick="ajaxpage2('../order/orderAddSecond.do?type=selectPrType','ajaxcontentarea',document.forms[0])"/>&nbsp;&nbsp;&nbsp;&nbsp;
			信件<html:radio property="prTypeId" value="8" onclick="ajaxpage2('../order/orderAddSecond.do?type=selectPrType','ajaxcontentarea',document.forms[0])"/>&nbsp;&nbsp;&nbsp;&nbsp;
			外呼<html:radio property="prTypeId" value="10" onclick="ajaxpage2('../order/orderAddSecond.do?type=selectPrType','ajaxcontentarea',document.forms[0])"/>&nbsp;&nbsp;&nbsp;&nbsp;
			<!--回单<html:radio property="prTypeId" value="11" onclick="ajaxpage2('../order/orderAddSecond.do?type=selectPrType','ajaxcontentarea',document.forms[0])"/>&nbsp;&nbsp;&nbsp;&nbsp;
			-->
		</td>
		
		<td colspan="2"><strong><font color="#990000">选择MSC</font></strong>&nbsp;&nbsp;
    	<html:select property="msc">
    	    <html:optionsCollection property="mscs" value="code" label="name"/> 
    	</html:select>
	</td>
	</tr>
  <tr> 
    <td>选择商品: 
      ...
      <!--
      <input name="queryItemQty" size="4" onblur="checkPositiveInteger(this);" value="1" maxlength="3"  onkeydown="add_item_f();">
      -->
	  ...
	  <logic:iterate id="msc" name="activeMsc">
		<input name="selectRGift" type="button" value=<bean:write name="msc" property="msc_Code"/>  onclick="openWin('../order/orderAddSecond.do?type=showRecruitGroupPage&isAdd=true&msc=<bean:write name="msc" property="msc_Code"/>','exchangeGift',700,500);" >&nbsp;
	  </logic:iterate>
	  </logic:equal>
	  
      <!--<input name="exchangeJFGift2" type="button" value="积分兑礼"  onclick="openWin('../order/orderAddSecond.do?type=showExchangePage&isAdd=true','exchangeGift',700,500);" > -->
...
<table width="95%" align="center" cellspacing="0" border="1" bordercolordark="#ffffff" bordercolorlight="#183648" >
  ...
  > <html:hidden name="item" property="sku_id"/>
    ...
    <td nowrap> 
      <input name="BtnDelete" value="删除" type="button" onClick=deleteItem(<%= item_index%>)>
	  ...
	  <logic:notEqual name="item" property="sellTypeId" value="-1">
      <input name="BtnUdate" value="确认" type="button" onClick="updateItem(<%= item_index%>,this)">
	  </logic:notEqual>
	</td>
  </tr>
  <% item_index++; %>
  ...
  > <html:hidden name="item" property="sku_id"/>
    ...
    <td align="right" id="itemMoney"><bean:write name="item" property="itemMoney" format="#0.00" ignore="true"/></td>
    <td nowrap> 
      <input name="BtnDelete" value="删除" type="button" onClick=deleteGift2(<%= item_index%>)>
      <logic:notEqual name="item" property="sku_id" value="0">
      <input name="BtnDelete" value="采购信息" type="button" onClick="pop_poinfo(<bean:write name="item" property="sku_id"/>)">
      </logic:notEqual>
	  <logic:notEqual name="item" property="sellTypeId" value="-1">
      <input name="BtnUdate" value="确认" type="button" onClick="updateGift2(<%= item_index%>,this)">
	  </logic:notEqual>
	</td>
  </tr>
  <% item_index++; %>
  ...
  > <html:hidden name="gift" property="sku_id"/>
    ...
    <td nowrap> 
      <input name="BtnDelete" value="删除" type="button" onClick="deleteGift(<%=gift_index%>,<bean:write name="gift" property="sellTypeId"/>);">
	    <logic:greaterEqual name="gift" property="sellTypeId" value="4">
    		<logic:notEqual name="gift" property="sellTypeId" value="4" >
    				<input name="combout" value="永久删除" type="button" onClick="comboutGift(<%=gift_index%>);">
				</logic:notEqual>
    	</logic:greaterEqual>
      <logic:notEqual name="gift" property="sku_id" value="0">
      <input name="BtnDelete2" value="采购信息" type="button" onClick=pop_poinfo(<bean:write name="gift" property="sku_id"/>)>
      </logic:notEqual>
   	  <input name="BtnDelete3" value="确认" type="button" onClick="updateGift(<%=gift_index%>,this);">
    </td>
  </tr>
  <% gift_index++; %>
  </logic:iterate>
  ...
  -->
    ...
    <td align="right"><bean:write name="ticket" property="money" format="#0.00"/>
  </td>
    <td width="80"><input name="" value="删除" type="button" onClick="deleteTicket('<bean:write name="ticket" property="ticketCode"/>');"></td>
  </tr>
  </logic:iterate>
  <tr> 
    <td colspan="1">礼券总计：</td>
    <td colspan="8" align="right">&nbsp;<bean:write name="cart" property="ticketKill" format="#0.00" ignore="true"/></td><td>&nbsp;</td>
  </tr>
</table>
...
<logic:iterate name="gifts2" id="gift2" type="com.magic.crm.order.entity.Proms2">
    ...
    <td align="left"><input type="button" value="购买" onclick="addGifts2(<bean:write name="item" property="awardId"/>)">
    </td>
    </tr>
    </logic:iterate>
</logic:iterate>
