...
function delete_f2(row) {
	...
	if (confirm("确实要删除吗？"))
	{
		frm.action="expExchangeActivity.do?type=deleteStep&forward=add_step&rowId=" + (row-1);
		frm.submit();
	}
...
function additem_f2() {
	var frm = document.forms[0];
	frm.action="expExchangeActivity.do?type=addStep&forward=add_step";
	frm.submit();
}
...
function prevStep() {
	var frm = document.forms[0];
	frm.action = "expExchangeActivity.do?type=backShowAddActivity";
	frm.submit();
}
function nextStep() {
	var frm = document.forms[0];
	frm.action = "expExchangeActivity.do?type=showAddStepGift";
	frm.submit();
}
</SCRIPT>
</head>
...
<table align="center" width="500" border="1" cellSpacing=0  cellpadding="3" id="detailTable">
