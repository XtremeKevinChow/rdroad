...
<form   action="sale_sku_detail.jsp" method="post" name="form" onsubmit="return querySubmit();">
	...
	<tr>
		...
		发送方式&nbsp;&nbsp;
		    <select name="delivery_type">
		      ...
		      <option value="3"<%if(delivery_type.equals("4")){%>selected<%}%>>邮政EMS</option>
		      <option value="3"<%if(delivery_type.equals("5")){%>selected<%}%>>货到付款快递</option>
		    </select>
		</td>
...
<table width="95%" align="center" cellspacing="1" border="0"  class="OraTableRowHeader" noWrap id="data">
	<tr">
		...
		<td width="20%" class="OraTableRowHeader" noWrap  noWrap align=middle>产品名称</td>
		<td width="10%" class="OraTableRowHeader" noWrap  noWrap align=middle>SKU</td>
...
<%
		...
		count_sql+=" c.itm_code,d.itm_name,b.sku_id,b.price,sum(b.quantity) sum_qty,sum(b.quantity*b.price) sum_price,sum(c.itm_cost*b.quantity) sum_cost, ";
		...
		count_sql+=" group by d.itm_type,c.itm_code,d.itm_name,b.sku_id,b.price ";

        request.getSession().setAttribute("excel_name","产品销售明细");
        request.getSession().setAttribute("excel_title","产品类型,货号,产品名称,SKU,销售价格,销售数量,销售金额,成本,毛利");
...
%>
	<tr>
		...
		<td bgcolor="#FFFFFF"><%=rs.getString("sku_id")%></td>		
