...
public class ConsoleDepositAction extends Action {
		...
		try {
			...
			if (iscallcenter != null && iscallcenter.equals("1")) {
				...
				if (hander.isOnService()) {
					int size = MemberaddMoneyDAO.countRecordsByCondition(conn, mb.getID());//计算记录总数
	                Pager page = new Pager(pageData.getOffset(), size);//生成page对象
	                page.setLength(10);
	                pageData.setPager(page);
	                coll = MemberaddMoneyDAO.getDepostHistoryByMember(conn, mb.getID(), pageData);
